...
def post_migrate_handler(sender, **kwargs):
    ...
    obj, __ = ModelLabelField.objects.update_or_create(name=f"*", field_type=field_type, defaults={'label': _("全部表")},
                                                      parent=None)
    ModelLabelField.objects.update_or_create(name=f"*", field_type=field_type, parent=obj,
                                             defaults={'label': _("全部字段")})
    ...
    for model in sender.models.values():
        ...
        if 'relationship' in verbose_name and '_' in model_name:
            continue
        obj, __ = ModelLabelField.objects.update_or_create(name=f"{label}.{model_name}", field_type=field_type,
