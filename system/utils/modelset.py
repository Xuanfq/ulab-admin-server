...
class ChangeRolePermissionAction(object):
    ...
    @swagger_auto_schema(request_body=openapi.Schema(
        ...
        properties={'roles': openapi.Schema(description = _("角色信息"), type=openapi.TYPE_ARRAY,
                                            items=openapi.Schema(type=openapi.TYPE_STRING, )),
                    'rules': openapi.Schema(description = _("数据权限"), type=openapi.TYPE_ARRAY,
                                            items=openapi.Schema(type=openapi.TYPE_STRING, )),
                    'mode_type': openapi.Schema(description = _("权限模式"), type=openapi.TYPE_NUMBER,
                                                default=ModeTypeAbstract.ModeChoices.OR)}
    ), operation_description = _("分配角色-数据权限"))
    ...
    def empower(self, request, *args, **kwargs):
        ...
        return ApiResponse(code=1004, detail = _("数据异常"))
...
class InvalidConfigCacheAction(object):
    ...
    @swagger_auto_schema(operation_description = _("使配置值缓存失效"), ignore_params=True)
    @action(methods=['post'], detail=True)
