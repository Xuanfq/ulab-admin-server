...
class ModelLabelFieldSerializer(BaseModelSerializer):
    ...
    field_type = LabeledChoiceField(choices=models.ModelLabelField.FieldChoices.choices,
                                    default=models.ModelLabelField.FieldChoices.DATA, label = _("字段类型"))
...
class RoleSerializer(BaseModelSerializer):
    ...
    menu = BasePrimaryKeyRelatedField(queryset=models.Menu.objects, many=True, label = _("菜单"), attrs=['pk', 'name'])
...
class DataPermissionSerializer(BaseModelSerializer):
    ...
    mode_type = LabeledChoiceField(choices=models.ModeTypeAbstract.ModeChoices.choices,
                                   default=models.ModeTypeAbstract.ModeChoices.OR, label = _("权限模式"))
...
class BaseRoleRuleInfo(BaseModelSerializer):
    roles = BasePrimaryKeyRelatedField(queryset=models.UserRole.objects, allow_null=True, required=False,
                                       attrs=['pk', 'name'], label = _("角色"), many=True)
    rules = BasePrimaryKeyRelatedField(queryset=models.DataPermission.objects, allow_null=True, required=False,
                                       attrs=['pk', 'name'], label = _("数据权限"), many=True)
    mode_type = LabeledChoiceField(choices=models.ModeTypeAbstract.ModeChoices.choices,
                                   default=models.ModeTypeAbstract.ModeChoices.OR.value, label = _("权限模式"))
...
class DeptSerializer(BaseRoleRuleInfo):
    ...
    user_count = serializers.SerializerMethodField(read_only=True, label = _("用户数量"))
    parent = BasePrimaryKeyRelatedField(queryset=models.DeptInfo.objects, allow_null=True, required=False,
                                        label = _("上级部门"), attrs=['pk', 'name'])
...
class UserSerializer(BaseRoleRuleInfo):
    ...
    dept = BasePrimaryKeyRelatedField(queryset=models.DeptInfo.objects, allow_null=True, required=False,
                                      attrs=['pk', 'name'], label = _("部门"))
    gender = LabeledChoiceField(choices=models.UserInfo.GenderChoices.choices,
                                default=models.UserInfo.GenderChoices.UNKNOWN, label = _("性别"))
...
class MenuSerializer(BaseModelSerializer):
    meta = MenuMetaSerializer(label = _("菜单元属性"))
    ...
    parent = BasePrimaryKeyRelatedField(queryset=models.Menu.objects, allow_null=True, required=False, label = _("上级菜单"),
                                        attrs=['pk', 'name'])
    model = BasePrimaryKeyRelatedField(queryset=models.ModelLabelField.objects, allow_null=True, required=False,
                                       label = _("绑定模型"), attrs=['pk', 'name'], many=True)

    menu_type = LabeledChoiceField(choices=models.Menu.MenuChoices.choices,
                                   default=models.Menu.MenuChoices.DIRECTORY, label = _("菜单类型"))
...
class MenuPermissionSerializer(MenuSerializer):
    ...
    title = serializers.CharField(source='meta.title', read_only=True, label = _("菜单名称"))


class RouteSerializer(MenuSerializer):
    meta = RouteMetaSerializer(all_fields=True, label = _("菜单元属性"))  # 用于前端菜单渲染
...
class OperationLogSerializer(BaseModelSerializer):
    ...
    creator = UserInfoSerializer(fields=['pk', 'username'], read_only=True, label = _("操作用户"))
    module = serializers.SerializerMethodField(label = _("访问模块"))
...
class NoticeMessageSerializer(BaseModelSerializer):
    ...
    notice_user = BasePrimaryKeyRelatedField(many=True, queryset=models.UserInfo.objects, label = _("被通知的用户"),
                                             attrs=['pk', 'username'])
    notice_dept = BasePrimaryKeyRelatedField(many=True, queryset=models.DeptInfo.objects, label = _("被通知的部门"),
                                             attrs=['pk', 'name'])
    notice_role = BasePrimaryKeyRelatedField(many=True, queryset=models.UserRole.objects, label = _("被通知的角色"),
                                             attrs=['pk', 'name'])

    files = serializers.JSONField(write_only=True, label = _("上传文件"))
    user_count = serializers.SerializerMethodField(read_only=True, label = _("用户数量"))
    read_user_count = serializers.SerializerMethodField(read_only=True, label = _("消息已读用户数量"))

    notice_type = LabeledChoiceField(choices=models.NoticeMessage.NoticeChoices.choices,
                                     default=models.NoticeMessage.NoticeChoices.USER, label = _("消息类型"))
...
class UserNoticeSerializer(BaseModelSerializer):
    ...
    notice_type = LabeledChoiceField(choices=models.NoticeMessage.NoticeChoices.choices,
                                     default=models.NoticeMessage.NoticeChoices.USER, label = _("消息类型"))
    unread = serializers.SerializerMethodField(label = _("消息是否未读"))
...
class NoticeUserReadMessageSerializer(BaseModelSerializer):
    ...
    owner_info = UserInfoSerializer(fields=['pk', 'username'], read_only=True, source='owner', label = _("已读用户"))
    notice_info = NoticeMessageSerializer(fields=['pk', 'level', 'title', 'notice_type', 'message', 'publish'],
                                          read_only=True, source='notice', label = _("消息公告"))
...
class SystemConfigSerializer(BaseModelSerializer):
    ...
    cache_value = serializers.SerializerMethodField(read_only=True, label = _("配置缓存数据"))
...
class UserPersonalConfigExportImportSerializer(SystemConfigSerializer):
    ...
    owner = BasePrimaryKeyRelatedField(attrs=['pk', 'username'], label = _("用户"), queryset=models.UserInfo.objects,
                                       required=True)
...
class UserPersonalConfigSerializer(SystemConfigSerializer):
    ...
    owner = BasePrimaryKeyRelatedField(attrs=['pk', 'username'], label = _("用户"), read_only=True)
    ...
    def create(self, validated_data):
        ...
        if not config_user and not owner:
            raise ValidationError(_("用户ID不能为空"))
        if owner:
...
class UserLoginLogSerializer(BaseModelSerializer):
    ...
    creator = BasePrimaryKeyRelatedField(attrs=['pk', 'username'], read_only=True, label = _("操作用户"))
