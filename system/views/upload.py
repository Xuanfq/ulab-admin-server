...
class UploadView(APIView):
    ...
    @swagger_auto_schema(manual_parameters=[
        openapi.Parameter(
            ...
            description = _("待上传的文件"), required=True
        ),
    ], operation_description = _("文件上传"))
        ...
        for file_obj in files:
            try:
                ...
                if file_obj.size > SysConfig.FILE_UPLOAD_SIZE:
                    return ApiResponse(code=1003, detail=_("文件大小不能超过") + f"{SysConfig.FILE_UPLOAD_SIZE}")
            except Exception as e:
                logger.error(f"user:{request.user} upload file type error Exception:{e}")
                return ApiResponse(code=1002, detail = _("错误的文件类型"))
