...
class RegisterView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not SysConfig.REGISTER:
            return ApiResponse(code=1001, detail = _("禁止注册"))
        ...
        if verify_token(token, client_id, success_once=True) and username and password:
            ...
            if UserInfo.objects.filter(username=username).count():
                return ApiResponse(code=1001, detail = _("用户名已经存在，请换个试试"))
        ...
        return ApiResponse(code=1001, detail=_("Token校验失败,请刷新页面重试"))
...
class LoginView(TokenObtainPairView):
    ...
    def post(self, request, *args, **kwargs):
        if not SysConfig.LOGIN:
            return ApiResponse(code=1001, detail = _("禁止登录"))
        ...
        if client_id and token and captcha_key and verify_token(token, client_id, success_once=True):
            ...
            if is_valid:
                ...
                return ApiResponse(code=9999, detail = _("验证码不正确，请重新输入"))

        return ApiResponse(code=9999, detail=_("Token校验失败,请刷新页面重试"))
...
class LogoutView(APIView):
    ...
    def post(self, request):
        """
        Log out of the account and add access and refresh tokens to the blacklist
        """
