...
class BaseFileRenderer(BaseRenderer):
    ...
    def compress_into_zip_file(self, value, request, response):
        ...
        if not secret_key:
            content = "{} - The encryption password has not been set - please go to personal information -> file encryption password to set the encryption password".format(request.user.name)
