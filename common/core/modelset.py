...
class UploadFileAction(object):
    ...
    ], operation_description = _("上传头像"), methods=['post'])
    ...
    def upload(self, request, *args, **kwargs):
        ...
        try:
            ...
            if file_obj.size > self.FILE_UPLOAD_SIZE:
                return ApiResponse(code=1003, detail=_('图片大小不能超过') + f" {self.FILE_UPLOAD_SIZE}")
        except Exception as e:
            return ApiResponse(code=1002, detail=_('错误的图片类型, 类型应该为') +f" {','.join(self.FILE_UPLOAD_TYPE)}")
...
class RankAction(object):
    ...
    ), operation_description = _("根据主键顺序，进行从小到大进行排序"))
    ...
    def action_rank(self, request, *args, **kwargs):
        ...
        for pk in pks:
            ...
            rank += 1
        return ApiResponse(detail = _("顺序保存成功"))
...
class ImportExportDataAction(OnlyExportDataAction):
    ...
    def import_data(self, request, *args, **kwargs):
        ...
        if act and request.data:
            ...
            return ApiResponse(detail = _("操作成功"))
        return ApiResponse(detail = _("数据异常"), code=1001)


class ChoicesAction(object):
    choices_models: []

    @swagger_auto_schema(operation_description = _("获取字段选择"), ignore_params=True, responses={
        200: openapi.Response(_("字段选择结果"), openapi.Schema(
            type=openapi.TYPE_OBJECT,
            properties={
...
class SearchFieldsAction(object):
    ...
    @swagger_auto_schema(operation_description = _("获取可查询字段"), ignore_params=True, responses={
        200: openapi.Response(_("可查询字段结果"), openapi.Schema(
            type=openapi.TYPE_OBJECT,
            properties={
                'code': openapi.Schema(type=openapi.TYPE_NUMBER, default=1000),
                'detail': openapi.Schema(type=openapi.TYPE_STRING, default='success'),
...
class BatchDeleteAction(object):
    ...
    def batch_delete(self, request, *args, **kwargs):
        ...
        if not pks:
            return ApiResponse(code=1003, detail = _("数据异常，批量操作主键列表不存在"))
        ...
        for instance in self.filter_queryset(self.get_queryset()).filter(pk__in=pks):
            try:
                ...
                pass
        return ApiResponse(detail=_("操作成功，批量删除数据数量：") + f"{count}")
